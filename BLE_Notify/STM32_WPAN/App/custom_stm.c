...
typedef struct{
  ...
  uint16_t  CustomMychar1Hdle;                  /**< myChar1 handle */
  uint16_t  CustomMynotifyHdle;                  /**< myNotify handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeMychar1 = 1;
uint16_t SizeMynotify = 1;
...
/* USER CODE BEGIN PV */
 uint8_t count = 0;
extern volatile uint8_t notify_enabled;
/* USER CODE END PV */
...
//	}
  if(notify_enabled)
  {
	...
	//Custom_STM_App_Update_Char_Ext(Connection_Handle, CUSTOM_STM_MYNOTIFY, &count);
	    Custom_STM_App_Update_Char(CUSTOM_STM_MYNOTIFY, &count);
...
#define COPY_MYCHAR1_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_MYNOTIFY_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x01,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomMynotifyHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_MYNOTIFY_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_MYNOTIFY_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomMychar1Hdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
            /* USER CODE BEGIN CUSTOM_STM_Service_1_Char_1_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */

            /* USER CODE END CUSTOM_STM_Service_1_Char_1_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */
          } /* if (attribute_modified->Attr_Handle == (CustomContext.CustomMychar1Hdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   *                                2 for myChar1 +
   *                                2 for myNotify +
   *                                1 for myNotify configuration descriptor +
   *                              = 6
   *
  ...
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: myService, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: myService , handle = 0x%04x \n\r", CustomContext.CustomMyserviceHdle);
  }

  /**
   *  myChar1
   */
  COPY_MYCHAR1_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomMyserviceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeMychar1,
                          CHAR_PROP_WRITE,
                          ...
                          &(CustomContext.CustomMychar1Hdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : MYCHAR1, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : MYCHAR1 , handle = 0x%04x \n\r", CustomContext.CustomMychar1Hdle);
  }
   ...
   *  myNotify
   */
  COPY_MYNOTIFY_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomMyserviceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeMynotify,
                          CHAR_PROP_NOTIFY,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(CustomContext.CustomMynotifyHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : MYNOTIFY, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : MYNOTIFY , handle = 0x%04x \n\r", CustomContext.CustomMynotifyHdle);
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_MYCHAR1:
      ret = aci_gatt_update_char_value(CustomContext.CustomMyserviceHdle,
                                       CustomContext.CustomMychar1Hdle,
                                       0, /* charValOffset */
                                       SizeMychar1, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value MYCHAR1 command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value MYCHAR1 command\n\r");
      }
    ...
    case CUSTOM_STM_MYNOTIFY:
      ret = aci_gatt_update_char_value(CustomContext.CustomMyserviceHdle,
                                       CustomContext.CustomMynotifyHdle,
                                       0, /* charValOffset */
                                       SizeMynotify, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value MYNOTIFY command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value MYNOTIFY command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_MYCHAR1:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMyserviceHdle, CustomContext.CustomMychar1Hdle, SizeMychar1, pPayload);
    ...
    case CUSTOM_STM_MYNOTIFY:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMyserviceHdle, CustomContext.CustomMynotifyHdle, SizeMynotify, pPayload);
