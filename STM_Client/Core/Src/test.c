...
typedef struct
{
  uint8_t Address[BD_ADDR_SIZE_LOCAL];
...
static DiscoveredDevice_t a_DiscoveredDevices[MAX_DISCOVERED_DEVICES];
static uint8_t NbDiscoveredDevices = 0;
/* USER CODE END PV */
...
/* USER CODE BEGIN PFP */
static int BLE_UpdateDiscoveredDeviceList(const uint8_t *addr, uint8_t addr_type,
          ...
          case HCI_LE_ADVERTISING_REPORT_SUBEVT_CODE:
            {
              ...
              char local_name[32] = {0};   /* buffer to hold decoded device name */
              uint8_t name_len = 0;        /* how many name bytes we actually got */
              uint8_t *report_addr;
              uint8_t report_addr_type;
              int8_t rssi;
              ...
              */
              ...
              /* USER CODE BEGIN EVT_LE_ADVERTISING_REPORT_2A */
              report_addr      = le_advertising_event->Advertising_Report[0].Address;
              report_addr_type = le_advertising_event->Advertising_Report[0].Address_Type;
              rssi             = (int8_t)*(uint8_t*)(adv_report_data + event_data_size);
              (void)event_type; /* no longer gates parsing below - see note; kept if you want to log PDU type */

              /* NOTE: we intentionally do NOT gate this loop on event_type == ADV_IND.
               * The AD-structure (TLV) format is identical for every report type, and
               * many peripherals only put the Complete/Shortened Local Name in the
               * SCAN_RSP packet rather than the primary ADV_IND - gating on ADV_IND
               * silently drops those names. */
              /* USER CODE END EVT_LE_ADVERTISING_REPORT_2A */
              ...
              while(k < event_data_size)
              {
                ...
                /* USER CODE BEGIN adlength_check */
                if (adlength == 0)
                {
                  break; /* malformed AD structure - stop instead of looping forever */
                }
                /* USER CODE END adlength_check */
                ...
                switch (adtype)
                {
                  ...
                  case AD_TYPE_MANUFACTURER_SPECIFIC_DATA: /* Manufacturer Specific */
                    ...
                    { /* ST VERSION ID 01 */
                      ...
                      {   /* Demo ID */
                         ...
                         SERVER_REMOTE_ADDR_TYPE = report_addr_type;
                         memcpy(SERVER_REMOTE_BDADDR, report_addr, BD_ADDR_SIZE_LOCAL);
                         break;
                  ...
                  case AD_TYPE_SHORTENED_LOCAL_NAME:
                    ...
                    {
                      name_len = sizeof(local_name) - 1; /* avoid overflowing the buffer */
                    }
              ...
              /* Merge this report into the running, deduplicated device table
               * (keyed by BD address) and print it. A device typically shows up
               * across two separate reports - ADV_IND then SCAN_RSP - so this
               * keeps the RSSI/name from both in sync under one entry. */
              (void)BLE_UpdateDiscoveredDeviceList(report_addr, report_addr_type, rssi,
                                                    name_len ? local_name : NULL);
...
/**
 * @brief  Find (or add) a discovered-device entry by BD address, update its
 *         RSSI, and merge in a decoded name if one was found in this report.
 *         Prints the (updated) entry to UART1 so you get a live list on boot.
 * @param  addr:      6-byte BD address from the advertising report
 * @param  addr_type: address type from the advertising report
 * @param  rssi:      RSSI decoded from this report
 * @param  name:      NUL-terminated name if one was parsed this report, else NULL
 * @retval index of the entry in the table, or -1 if the table is full
 */
static int BLE_UpdateDiscoveredDeviceList(const uint8_t *addr, uint8_t addr_type,
                                           int8_t rssi, const char *name)
{
  ...
  for (i = 0; i < NbDiscoveredDevices; i++)
  {
    if (memcmp(a_DiscoveredDevices[i].Address, addr, BD_ADDR_SIZE_LOCAL) == 0)
    {
      idx = i;
      break;
    }
  ...
  if (idx < 0)
  {
    if (NbDiscoveredDevices >= MAX_DISCOVERED_DEVICES)
    {
      return -1; /* table full - drop silently rather than corrupt memory */
    }
    idx = NbDiscoveredDevices++;
    memcpy(a_DiscoveredDevices[idx].Address, addr, BD_ADDR_SIZE_LOCAL);
    a_DiscoveredDevices[idx].Address_Type = addr_type;
    a_DiscoveredDevices[idx].NameValid = 0;
    a_DiscoveredDevices[idx].Name[0] = '\0';
  }

  a_DiscoveredDevices[idx].Rssi = rssi;

  if (name != NULL)
  {
    strncpy(a_DiscoveredDevices[idx].Name, name, sizeof(a_DiscoveredDevices[idx].Name) - 1);
    a_DiscoveredDevices[idx].Name[sizeof(a_DiscoveredDevices[idx].Name) - 1] = '\0';
    a_DiscoveredDevices[idx].NameValid = 1;
  }

  printf("[%2d] %02X:%02X:%02X:%02X:%02X:%02X | RSSI %4d dBm | %s\r\n",
         idx,
         a_DiscoveredDevices[idx].Address[5], a_DiscoveredDevices[idx].Address[4],
         a_DiscoveredDevices[idx].Address[3], a_DiscoveredDevices[idx].Address[2],
         a_DiscoveredDevices[idx].Address[1], a_DiscoveredDevices[idx].Address[0],
         a_DiscoveredDevices[idx].Rssi,
         a_DiscoveredDevices[idx].NameValid ? a_DiscoveredDevices[idx].Name : "(no name)");
